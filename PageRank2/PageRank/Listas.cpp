...
void Listas::INSALALISTA(Node *&BODYS, string NEWONE) {
...
void  Listas::SEARCHINSALALISTA(const string &INFORMACIONESPECIAL, char * PALABRACLAVE, string DATOSDDENTRODELISTA, int CONTEO) {
	...
	if (DIR *direccionamiento = opendir("PageRank")) {
		while (dirent *ENTRADADATOS = readdir(direccionamiento)) {
			EL = ENTRADADATOS->d_name;
			if (EL != "." && EL != "..") {
				if (INFORMACIONESPECIAL.find(VAPLCLASE) != string::npos) {
					if (INFORMACIONESPECIAL.find(EL) != string::npos) {
						cout << DATOSDDENTRODELISTA << "Points to " << EL << "\n";
						INSALALISTA(LISTAADYACENCIA[CONTEO], EL);
					}
				}FILES.close();
			}
		}closedir(direccionamiento);
	}
}
void  Listas::READING(char *PALABRACLAVE) {
	...
	if (DIR*direccionamiento = opendir("PageRank")) {
		while (ENTRADAYDALIDA = readdir(direccionamiento)) {
			DATOS = ENTRADAYDALIDA->d_name; if (DATOS != "." && DATOS != "..") {
				FILES.open("PageRank//" + DATOS, ios::in);
				if (!FILES) { cerr << "No se pudo encontrar el archivo que se necesita :I "; return; }
				LISTAADYACENCIA[CONTEO] = new Node(DATOS);
				while (!FILES.eof()) {
					string GETGA, contenido; while (getline(FILES, GETGA)) { contenido += GETGA + "\n"; }
...
void Listas::IMPRIMIRLISTACONSOLEMODE() {
